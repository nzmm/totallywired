...
export const queryReleases = (artistName: string, releaseName: string) => {
  return getMbz<MBReleaseQueryResponse>("/release", new URLSearchParams({
 ...
 */
export const getRelease = (releaseId: string, inc = "recordings+artist-credits+labels") => {
  return getMbz<MBReleaseResponse>(`/release/${releaseId}`, new URLSearchParams({
    inc,
    fmt: 'json'
  }));
}
 ...
 */
export const getCoverArtUrl = (releaseId: string, size: 250 | 500 | 1200 = 250) => {
  return getArt(`/release/${releaseId}/front-${size}`)
    .then(r => r.ok ? r.data?.url ?? DEFAULT_COVERART_URL : DEFAULT_COVERART_URL)
